    ...
    public void driveCars()
    {
        while (cars.size() > 0)
        {
            ...
            while (i < cars.size())
            {
                ...
                {
                    cars.remove(i);
                    for (int j = 0; j<c.passengers.size();j++){
                        if (c.passengers.get(j).getDestination() == c.destination){
                            System.out.println(c.getDriverName() + " drops off " + c.passengers.get(j).getName());
                        }
                    }
